...
class VMLAlgorithmQuerySet(models.query.QuerySet):
...
class VMLAlgorithm(models.Model):
    '''
    ...
    owner = models.CharField(max_length=128)
    created_at = models.DateTimeField(auto_now_add=True, blank=True)
    parent_endpoint = models.ForeignKey(VEndpoint, on_delete=models.CASCADE)
    objects = VMLAlgorithmQuerySet.as_manager()

class VMLAlgorithmStatus(models.Model):
    '''
    The MLAlgorithmStatus represent status of the MLAlgorithm which can change during the time.

    Attributes:
        status: The status of algorithm in the endpoint.
    ...
    created_at = models.DateTimeField(auto_now_add=True, blank=True)
    parent_mlalgorithm = models.ForeignKey(VMLAlgorithm, on_delete=models.CASCADE, related_name = "status")


class VMLRequest(models.Model):
    '''
