...
class JacobianInverseKinematicsCK:
    """
    Jacobian Based Full Body IK Solver with a secondary task that imitates the input quaternions 
    Batch-mode motion retargetting by Kwang-Jin Choi and Hyeong-Seok Ko
    ...
    references : {int : (F, 3) ndarray}
        List of J joint rotation
        references for which the result
        should bias toward
    ...
    def __init__(self, animation, targets,
        references=None, iterations=10,
        recalculate=True, damping=2.0,
        secondary=0.25, smoothness=0.0, translate=False, angle_limits=None,
        silent=False, weights=None,
        weights_translate=None):
    ...
    def __call__(self, descendants=None, gamma=1.0):
        ...
        for i in range(self.iterations):
            ...
            dx1 = np.array(list(map(lambda jf, ef:
                linalg.lu_solve(linalg.lu_factor(jf.T.dot(jf) + d), jf.T.dot(ef)), j, e)))
            
            dx2 = 0*dx1;

            xp = np.vstack((x[0], x[:nf-1]))
            #print(xp.shape)
            xa = np.vstack((x[1:], x[nf-1]))
            #print(xa.shape)
            
            #ns = np.array(list(map(lambda jf:
            #                       np.eye(x.shape[1]) - linalg.solve(jf.T.dot(jf) + d, jf.T.dot(jf)), j)))
            #dx2 = self.smoothness*np.array(list(map(lambda nsf, xpf, xf, xaf: nsf.dot(xpf + xaf - 2*xf), ns, xp, x, xa)))

            #dx2[:,:nj*3] = 0.05*np.array(list(map(lambda nsf, xpf, xf, xaf: nsf[:nj*3,:nj*3].dot(xpf[:nj*3] + xaf[:nj*3] - 2*xf[:nj*3]), ns, xp, x, xa)))
            #dx2[:,:nj*3] = 0.05*np.array(list(map(lambda xpf, xf, xaf: xpf[:nj*3] + xaf[:nj*3] - 2*xf[:nj*3], xp, x, xa)))
            dx2 = self.smoothness*np.array(list(map(lambda xpf, xf, xaf: xpf + xaf - 2*xf, xp, x, xa)))

            dx3 = 0*dx1
            """ Generate Secondary task """
            if self.references is not None:                
                ns = np.array(list(map(lambda jf:
                    np.eye(x.shape[1]) - linalg.solve(jf.T.dot(jf) + d, jf.T.dot(jf)), j)))
                
                dx3[:,:nj*3] = np.array(list(map(lambda nsf, yf, xf:
                                                    nsf[:nj*3,:nj*3].dot(yf - xf[:nj*3]), ns, y, x)))
                #dx3 = np.array(list(map(lambda nsf, yf, xf: nsf.dot(yf - xf), ns, y, x)))
            
            x += dx1 + dx2 + dx3

            if self.angle_limits is not None:
                for t in range(x.shape[0]):
                    x[t,:nj*3] = np.maximum(x[t,:nj*3], self.angle_limits[0])
                    x[t,:nj*3] = np.minimum(x[t,:nj*3], self.angle_limits[1])
                    #x[t,3*(6-1)+1] = 0.0
                    #x[t,3*(10-1)+1] = 0.0
            ...
            if not self.silent:
                ...
                gp = gp[:,np.array(list(self.targets.keys()))]
                error = np.mean(np.sum((self.endeff - gp)**2.0, axis=2)**0.5)
                print('[JacobianInverseKinematicsCK] Iteration %i | Error: %f' % (i+1, error))
...
class BasicJacobianIK:
    ...
    def __init__(self, animation, positions, iterations=10, silent=True, **kw):
        ...
        self.ik = JacobianInverseKinematicsCK(animation, targets, iterations=iterations, silent=silent, **kw)
